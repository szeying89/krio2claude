...
- [ ] **Task 7: Agent/LLM gateway with determinism controls and trajectory cache**
  ...
  Provider-agnostic gateway (Anthropic/OpenAI/Bedrock, config-selected) with pinned temperature 0 and params, versioned prompt templates, strict Pydantic validation with bounded repair-retries, token/cost accounting, and a content-addressed cache keyed on `hash(prompt/tool-call-sequence + model_id + params + kb_snapshot + cri_snapshot)`. For agent loops (Task 1b), the cache stores the full trajectory (ordered tool calls + final output), not just a single prompt/response pair. Keys from env/keyring, redacted everywhere. Optional pre-send redaction pass with diff preview and a Mermaid-only deterministic mode.

  *Owner: Agent/LLM Gateway Service (used by every agent).*

  Tests: cache hit is byte-identical with zero network calls, for both single-prompt and full-trajectory cache entries; schema violation triggers repair then hard-fails retaining the raw response; keys never reach logs, DB, or serialised errors; redaction removes seeded secrets and is reversible for display.

  Demo: Same prompt twice — first records cost and latency, second returns instantly with a cache-hit badge; run an agent loop twice with identical inputs and see the second run replay its entire trajectory from cache; toggle redaction to preview exactly what would leave the machine.
